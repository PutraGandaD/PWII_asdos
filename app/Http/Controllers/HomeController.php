...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $grafik_jk_prodi = DB::select("SELECT prodis.nama, SUM(CASE WHEN mahasiswas.jk = 'L' THEN 1 ELSE 0 END) as laki, SUM(CASE WHEN mahasiswas.jk = 'P' THEN 1 ELSE 0 END) as perempuan FROM `prodis`
        JOIN mahasiswas ON prodis.id = mahasiswas.prodi_id
        GROUP BY prodis.nama");
